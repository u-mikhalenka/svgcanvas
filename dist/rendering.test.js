(function () {
  ...
  class ImageUtils {
    ...
    toDataURL(svgNode, width, height, type, encoderOptions, options) {
      ...
      if (type === 'image/jpeg' || type === 'image/png') {
        if (!options.async) {
          throw new Error(
            'svgcanvas: options.async must be set to true if type is image/jpeg | image/png',
          );
        }
        return (async () => {
      ...
      throw new Error(
        'svgcanvas: Unknown type for toDataURL, please use image/jpeg | image/png | image/svg+xml.',
      );
    }
  ...
  var Context = (function () {
    ...
    Context.prototype.rotate = function (angle) {
      let matrix = this.getTransform().multiply(
        new DOMMatrix([Math.cos(angle), Math.sin(angle), -Math.sin(angle), Math.cos(angle), 0, 0]),
      );
      this.setTransform(matrix);
    };
